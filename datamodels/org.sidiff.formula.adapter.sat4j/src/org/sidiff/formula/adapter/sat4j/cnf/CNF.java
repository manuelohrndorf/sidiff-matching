...
public class CNF {
   ...
   */
  public static Formula var(String label, Var name) {
    if (name == null) {
      throw new NullPointerException("Variablenname darf nicht `null' sein!");
    }
    return new FormulaVar(label, name);
  }
   ...
   *         unerfuellbar ist.
   */
  public static Set<String> satisfiable(Formula f) throws TimeoutException {
    TseitinVisitor tseitinVisitor = new TseitinVisitor();
    Integer x = f.accept(tseitinVisitor);
    ...
    if (problem.isSatisfiable()) {
      ...
      Set<String> trueVars = new HashSet<String>();
      for (Integer y : model) {
        if (y > 0) {
          trueVars.add(tseitinVisitor.getVars().get(y));
        }
      }
      return trueVars;
    } else {
